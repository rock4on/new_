#!/usr/bin/env python3
"""
End-to-end test for Inteligent Document Reader - Electricity ingestion and Excel matching
"""
...
try:
    ...
    from inteligent_document_reader.tools.matching_tool_electricity import MatchingToolElectricity
    from inteligent_document_reader.config import Config
except ImportError:
    # If running from within the directory, use relative imports
    from agents.document_agent import DocumentAgent
    from tools.matching_tool_electricity import MatchingToolElectricity
    from config import Config


def test_electricity_ingestion():
    """Test electricity document ingestion workflow"""
    print("🧪 TEST 1: ELECTRICITY DOCUMENT INGESTION")
    print("=" * 50)
    ...
    try:
        ...
        print(f"📁 Electricity folder: {Config.ELECTRICITY_FOLDER}")
        
        # Check if electricity folder exists and has PDF files
        electricity_path = Path(Config.ELECTRICITY_FOLDER)
        if not electricity_path.exists():
            print(f"⚠️  Electricity folder doesn't exist: {Config.ELECTRICITY_FOLDER}")
            print(f"   Creating folder for testing...")
            electricity_path.mkdir(parents=True, exist_ok=True)
            print(f"   📝 Add PDF electricity documents to {Config.ELECTRICITY_FOLDER} for testing")
            return False
        
        # Find PDF files
        pdf_files = list(electricity_path.rglob("*.pdf"))
        if not pdf_files:
            print(f"⚠️  No PDF files found in {Config.ELECTRICITY_FOLDER}")
            print(f"   📝 Add PDF electricity documents to {Config.ELECTRICITY_FOLDER} for testing")
            return False
        ...
        test_folder = Path("./test_electricity_batch")
        ...
        if batch_result.get('successful', 0) > 0:
            print("✅ Electricity ingestion test: PASSED")
            return True
        else:
            print("❌ Electricity ingestion test: FAILED")
            return False
            
    except Exception as e:
        print(f"❌ Electricity ingestion test error: {e}")
        return False


def test_excel_matching():
    """Test Excel matching with electricity database"""
    print("\n🧪 TEST 2: EXCEL ELECTRICITY MATCHING")
    print("=" * 50)
    
    try:
        # Check if Electricity_example.xlsm exists
        excel_file = Path("Electricity_example.xlsm")
        ...
        matching_tool = MatchingToolElectricity(search_client=search_client)
        ...
        matching_input = json.dumps({
            ...
            'output_excel_path': 'output_electricity.xlsm'
        })
        ...
        output_file = Path("output_electricity.xlsm")
...
def main():
    ...
    print("🚀 INTELIGENT DOCUMENT READER - END-TO-END TESTS (ELECTRICITY)")
    print("=" * 60)
    ...
    # Test 1: Electricity ingestion
    test_results.append(test_electricity_ingestion())
    ...
    print(f"\n📝 Test Details:")
    tests = ["Electricity Ingestion", "Excel Matching"]
