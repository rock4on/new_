"""
Natural Gas Matching Tool for Excel validation - reads input Excel and writes output Excel with natural gas matches
"""
...
class MatchingToolNaturalGas(BaseTool):
    """Tool for matching Excel data with natural gas documents and outputting to Excel format"""
   
    name: str = "excel_natural_gas_matching_tool"
    description: str = "Reads input Excel file (rows 6+, columns B-J), matches locations with natural gas database, writes results to output_natural_gas.xlsm (columns K-Q). Input should be JSON with 'input_excel_path'."
    ...
    def _search_natural_gas_by_location(self, location: str):
        """Search natural gas database for location matches"""
        try:
            print(f"   🔍 Searching natural gas database for: {location}")
           
            # Search for natural gas documents with matching location
            search_results = self.search_client.search(
                search_text=f"{location}",
                filter="doc_type eq 'NaturalGas'",  # Only natural gas documents
            ...
            try:
                filtered_results = self.search_client.search(
                    search_text="*",
                    filter=f"doc_type eq 'NaturalGas' and client_name eq '{location}'",
                    select=["id", "filename", "location", "client_name", "start_date",
                           "end_date", "consumption", "consumption_unit", "cost", "cost_unit", "processed_at"],
                    top=50
                )
    ...
    def _run(self, input_data: str) -> str:
        """Process Excel file and create output with natural gas matches"""
        try:
            data = json.loads(input_data)
            input_excel_path = data.get('input_excel_path', 'Gas_example.xlsm')
            output_excel_path = data.get('output_excel_path', 'output_natural_gas.xlsm')
           
            print(f"📊 NATURAL GAS MATCHING TOOL")
...
📍 Row {row_idx + 1}: Processing location '{location}'")
                ...
                # Search for natural gas matches
                natural_gas_match = self._search_natural_gas_by_location(str(location))
               
                if natural_gas_match:
                    matched_rows += 1
                    print(natural_gas_match)
                    # Write natural gas data to columns K-Q (indices 10-16)
                    df_output.iloc[row_idx, 10] = "Natural Gas Bill"  # Column K
                    df_output.iloc[row_idx, 11] = natural_gas_match.get('location', '')  # Column L
                    df_output.iloc[row_idx, 12] = natural_gas_match.get('start_date', '')  # Column M
                    df_output.iloc[row_idx, 13] = natural_gas_match.get('end_date', '')  # Column N
                    df_output.iloc[row_idx, 14] = natural_gas_match.get('consumption', '')  # Column O
                    df_output.iloc[row_idx, 15] = natural_gas_match.get('consumption_unit', '')  # Column P
                    df_output.iloc[row_idx, 16] = natural_gas_match.get('cost', '')  # Column Q
                    df_output.iloc[row_idx, 17] = natural_gas_match.get('cost_unit', '') # Column R
 
                    print(f"   ✅ Matched with natural gas: {natural_gas_match.get('filename', 'Unknown')}")
                    print(f"      Location: {natural_gas_match.get('location', 'N/A')}")
                    print(f"      Period: {natural_gas_match.get('start_date', 'N/A')} to {natural_gas_match.get('end_date', 'N/A')}")
                    print(f"      Consumption: {natural_gas_match.get('consumption', 'N/A')} {natural_gas_match.get('consumption_unit', '')}")
                    print(f"      Cost: {natural_gas_match.get('cost', 'N/A')} {natural_gas_match.get('cost_unit', '')}")
                else:
                    # Clear columns K-Q if no match
                    for col_idx in range(10, 18):
                        df_output.iloc[row_idx, col_idx] = ''
                    print(f"   ❌ No natural gas match found")
...
💾 Saved output to: {output_excel_path}")
            ...
            # Summary
            result = f"📊 NATURAL GAS MATCHING COMPLETED
"
            result += f"{'='*40}
"
...
"
            result += f"   • Rows with natural gas matches: {matched_rows}
"
            result += f"   • Match rate: {(matched_rows/processed_rows*100):.1f}% 
" if processed_rows > 0 else "   • Match rate: 0%
"
            result += f"   • Output columns: K-R populated with natural gas data
"
            result += f"
✅ Results written to {output_excel_path}"
            ...
            return "❌ Error: Invalid JSON input format. Use: {'input_excel_path': 'Gas_example.xlsm':}"
        except Exception as e:
